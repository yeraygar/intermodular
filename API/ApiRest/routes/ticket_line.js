...
router.put("/ticket_line/:id", (req, res) => {
    const {id} = req.params;
    const {name, cantidad, precio, stock, total, id_client, id_familia, id_ticket} = req.body;
    ticketLineSchema
        .updateOne({_id: id}, {$set:{name, cantidad, precio, stock, total, id_client, id_familia, id_ticket}})
        .then((data) =>{
            res.json(data);
