...
router.put("/tables/:id", (req, res) => {
    const {id} = req.params;
    const {name, status, ocupada, id_client, id_zone, id_row, id_column, comensales, id_user, id_ticket} = req.body;
    tableSchema
        .updateOne({_id: id}, {$set:{name, status, ocupada, id_client, id_zone, id_row, id_column, comensales, id_user, id_ticket}})
        .then((data) =>{
            res.json(data);
            console.log(`\nUpdate succesful: \n ${data}`);
        })
